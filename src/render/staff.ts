...
// 符头切点偏移：noteheadBlack 是倾斜椭圆(\),最外缘(切点)不在垂直中心，偏上(stemUp右切点)/偏下(stemDown左切点)。
// 像素实测(font-size=600)：切点偏移 24px / 墨迹高 150px = 0.16。
// stemUp 符干底端 = headY - 此偏移；stemDown 符干顶端 = headY + 此偏移。
// 这样符干连符头的端在切点高度，符头完全覆盖符干(不漏出)。
const HEAD_TANGENT_OFFSET = 0.16;  // 占墨迹高度的比例

// 线宽常量（staff space 单位，遵循 SMuFL engravingDefaults 推荐值）。
// staff line 0.13；barline 细 0.16 / 粗(终止) 0.5；ledger line 0.16；stem 0.24(加粗一倍,原 SMuFL 0.12 偏细)。
const W_STAFF = 0.13;
 ...
 *  符干边缘对齐符头墨迹最外缘(切点)：stemUp 符干右缘=符头右缘，stemDown 符干左缘=符头左缘。
 *  符干连符头的端在切点高度(偏上/偏下)，保证符头完全覆盖符干不漏出。 */
function computeStem(step: number, x: number, headHalfW: number, layout: Layout, beam: BeamCtx | undefined): {
  ...
  // 切点偏移：符头最外缘偏离垂直中心的高度(占墨迹高 ~16%)。墨迹高≈ss·1.0，故 ≈0.16·ss。
  const tangentOff = HEAD_TANGENT_OFFSET * ss;
  if (beam) {
    const stemUp = beam.stemDir === 'up';
    // 符干外侧边缘对齐符头墨迹最外缘：stemUp 右缘=x+headHalfW，stemDown 左缘=x-headHalfW
    const stemX = stemUp ? x + headHalfW - stemW : x - headHalfW;
    if (stemUp) {
      // 符干底端在右切点(偏上)，顶端在梁端点
      return { stemUp, stemW, stemX, stemTop: beam.stemEndY, stemBot: headY - tangentOff };
    } else {
      // 符干顶端在左切点(偏下)，底端在梁端点
      return { stemUp, stemW, stemX, stemTop: headY + tangentOff, stemBot: beam.stemEndY };
    }
  }
  // 无连梁：原有逻辑
  const stemUp = step <= 6;
  ...
  return {
    stemUp, stemW, stemX,
    stemTop: stemUp ? headY - tangentOff - stemLen : headY + tangentOff,
    stemBot: stemUp ? headY - tangentOff : headY + tangentOff + stemLen,
  };
}
...
function renderChordStems(piece: Piece, layout: Layout, beamIdx: Set<number>): { svg: string; handled: Set<number> } {
  ...
  const stemW = stemWidth(ss);
  const headHalfW = advanceSS('noteheadBlack') * INK_HALF_W_RATIO * ss;  // 墨迹半宽(与renderNote一致)
