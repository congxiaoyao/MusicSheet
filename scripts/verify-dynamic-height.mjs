...
// 中音区默认值(无扩展):height=259,staffTop=75,jianpuTop=165
...
// C4(step-2)有下加线 → 简谱下移到 155.5;E5 在五线谱内不触发顶部
check('A1 C4下加线 jianpuTop=155.5', lMid.jianpuTop, 155.5);
...
check('A3 A0 jianpuTop>165(简谱下移)', lLow.jianpuTop > 165, true);
check('A3 A0 height>259(底部扩展)', lLow.height > 259, true);
...
check('A4 两端 jianpuTop>165', lBoth.jianpuTop > 165, true);
...
pB1.notes.pop();  // 删 C8
const lB1b = lay(pB1);
check('B1 [C8]扩展→删C8 height 回 259', lB1b.height, 259);
...
pB2.notes.pop();
const lB2 = lay(pB2);
check('B2 [A0]→删A0 height 回 259', lB2.height, 259);
check('B2 删A0 jianpuTop 回 165', lB2.jianpuTop, 165);
...
check('B3 [C8,A0]删C8 底部仍扩展(jianpuTop>165)', lB3b.jianpuTop > 165, true);
...
check('B4 [C8,A0]删A0 底部回缩(jianpuTop=165)', lB4.jianpuTop, 165);
...
pB5.notes.pop();
const lB5 = lay(pB5);
check('B5 空谱 height=259', lB5.height, 259);
check('B5 空谱 viewBoxYOffset=0', lB5.viewBoxYOffset, 0);
check('B5 空谱 jianpuTop=165', lB5.jianpuTop, 165);

// B6: [C8, C4] → 删 C8 → 按中音 C4 重算(C4 有下加线,简谱仍下移到 155.5)
...
check('B6 C4有下加线 简谱下移 jianpuTop=155.5', lB6.jianpuTop, 155.5);
...
// C2: 和弦 [A0, C4, E5] → 删 A0 → 按 C4/E5 重算(C4 有下加线,简谱仍下移)
...
const lC2 = lay(pC2);
check('C2 [A0,C4,E5]删A0 jianpuTop=155.5(C4下加线)', lC2.jianpuTop, 155.5);
...
check('C3 底部回缩 jianpuTop=165', lC3.jianpuTop, 165);
...
check('D2 [C4]+hover A0 jianpuTop>165', lD2.jianpuTop > 165, true);
// D3: hover 移除 → 按 notes 重算(C4 有下加线,jianpuTop=155.5)
const lD3 = lay(pD2, undefined);
check('D3 hover移除 jianpuTop=155.5(C4下加线)', lD3.jianpuTop, 155.5);
